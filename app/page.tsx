...
export default async function HomePage() {
  ...
  const { data: children, error: childError } = await supabase.from('children').select('*').order('created_at').limit(1);
  if (childError) {
    return <main className="p-6 text-rose-300">Ошибка загрузки ребёнка: {childError.message}</main>;
  }
      ...
      <main className="mx-auto max-w-xl p-6">
        <div className="rounded-3xl border bg-slate-900 p-6">
          ...
          <p className="mt-3 text-slate-300">
            Добавьте ребёнка через Supabase SQL Editor. Пример есть в README и в supabase/schema.sql.
          </p>
        </div>
      </main>
    );
  ...
  const [{ data: feedings, error: feedError }, { data: sleeps, error: sleepError }] = await Promise.all([
  ...
  if (feedError || sleepError) {
    return <main className="p-6 text-rose-300">Ошибка загрузки событий: {(feedError ?? sleepError)?.message}</main>;
  }
